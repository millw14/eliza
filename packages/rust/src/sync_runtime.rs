...
// Use tokio::sync::RwLock only when not sync AND native feature is enabled on native targets
#[cfg(all(not(feature = "sync"), feature = "native", not(target_arch = "wasm32")))]
use tokio::sync::RwLock;

// Use std::sync::RwLock when sync feature is enabled, native is unavailable, or on wasm32
